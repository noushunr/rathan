...
import com.example.rathaanelectronics.ui.Adapter.WalletAdapter
...
class MyWalletFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_my_wallet, container, false)
    ...
    private fun getMyWallet() {
        ...
        val call: Call<WalletModel> = apiService.getMyWallet(
                    ...
                    if (status == "true") {
                        if (response.body()?.data != null) {
                            if (!response.body()?.data?.details?.isNullOrEmpty()!!){
                                ...
                                var walletAdapter = WalletAdapter(requireContext(),response.body()?.data?.details!!)
                                rvWallet.adapter = walletAdapter
                            }
                            try {
                                if (response.body()?.data?.totalDebits!=null){
                                    tvWithdraw.text = "%.2f".format(response.body()?.data?.totalDebits?.toDouble())
                                }
                                if (response.body()?.data?.totalCredits!=null){
                                    tvBalance.text = "%.2f".format(response.body()?.data?.totalCredits?.toDouble()
                                        ?.minus(response.body()?.data?.totalDebits?.toDouble()!!))
                                }
                            }catch (e:Exception){
