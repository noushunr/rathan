...
//        }
        ...
        holder.ivCart.setOnClickListener {
            Hotdeals_data[position].productId?.let { it1 ->
...
//            Log.e("image_url", ApiConstants.IMAGE_BASE_URL + get.prodFrondImg)
            ...
            if (get?.hotPrice != null) {
                if (get?.hotPrice?.toDouble()?.toInt() != 0) {
                    var offerPercentage =
                        ((get?.productSellPrice?.toDouble())?.minus((get?.hotPrice?.toDouble()!!)))?.div(
                    ...
                    if (roundOff?.toInt() != 0) {
                        ...
                        if (offerPercentage != null) {
                            txt_offer_float.text =
                                "${roundOff?.toInt()}% ${context?.getString(R.string.offer)}"
                        }
